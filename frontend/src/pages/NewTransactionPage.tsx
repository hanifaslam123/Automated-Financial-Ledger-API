...
const NewTransactionPage: React.FC = () => {
    ...
    return (
      <div className="max-w-lg mx-auto">
        <div className="card text-center space-y-4">
          ...
          <h2 className="text-xl font-bold text-slate-900">Transaction Posted!</h2>
          <p className="text-slate-600 text-sm">
            Reference: <span className="font-mono font-semibold text-slate-800">{refNum}</span>
          </p>
          <p className="text-slate-500 text-sm">
            The double-entry transaction has been recorded with 100% data accuracy.
          </p>
          <div className="flex gap-3 justify-center pt-2">
            <button onClick={() => { setSuccess(false); setForm({ debitAccountId: '', creditAccountId: '', amount: '', description: '', currency: 'USD' }); }} className="btn-secondary text-sm">
              New Transaction
            </button>
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      {/* Header */}
      <div>
        <Link to="/transactions" className="inline-flex items-center gap-1 text-sm text-slate-500 hover:text-slate-700 mb-3">
      ...
      <form onSubmit={handleSubmit} className="card space-y-5">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div className="p-4 bg-red-50 border border-red-200 rounded-xl">
            <p className="text-xs font-bold text-red-700 uppercase tracking-wider mb-2">← Debit (From)</p>
            <label className="label text-xs">Account</label>
            <select
              className="input text-sm"
              value={form.debitAccountId}
              onChange={set('debitAccountId')}
              required
          ...
          <div className="p-4 bg-green-50 border border-green-200 rounded-xl">
            <p className="text-xs font-bold text-green-700 uppercase tracking-wider mb-2">Credit (To) →</p>
            <label className="label text-xs">Account</label>
            <select
              className="input text-sm"
              value={form.creditAccountId}
              onChange={set('creditAccountId')}
              required
              disabled={loadingAccounts}
            >
              <option value="">Select account...</option>
        ...
        {form.debitAccountId && form.creditAccountId && form.amount && (
          <div className="flex items-center justify-center gap-2 py-2">
            ...
            <span className="text-xs text-green-600 font-semibold ml-1">✓ Balanced</span>
          </div>
        )}
